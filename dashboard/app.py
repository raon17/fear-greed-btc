...
# Without this, 'from src.fetch import ...' fails when run from dashboard/
...
st.title("Crypto Fear & Greed — does the contrarian signal work?")
st.caption("Backtesting the Fear & Greed Index as a BTC buy signal against buy-and-hold.")


# ── Live current reading ───────────────────────────────────────────────────────
# Fetch just today's single reading to show at the top
today = fetch_fear_greed(limit=1).iloc[0]
...
threshold = col_b.slider(
    "Buy signal: enter when Fear & Greed is below...",
    min_value=10,
    max_value=40,
    value=25,
    step=5,
...
# load_data() uses the cached CSV so this is fast after the first run
df     = load_data()
...
# ── Zone colours -- used across all charts ─────────────────────────────────────
zone_colors = {
    "Extreme Fear":  "#E24B4A",
    "Fear":          "#EF9F27",
    "Neutral":       "#888780",
...
# ── Chart 5: Scatter -- F&G value vs forward return ───────────────────────────
st.subheader(f"Scatter: F&G value vs {horizon}-day return")
st.caption("If the signal works, you'd expect lower F&G values (left side) to cluster higher on the y-axis. Expect noise.")
